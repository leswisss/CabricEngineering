        ...
        <div className={styles.cc__bottom}>
          {contactDetails.map((data, i) => (
            <div key={i} className={styles.ccb__wrap}>
              <p className={styles.p__country}>{data.country}</p>
              <div className={styles.cc__svg}>
                <data.svg />
              </div>
              <div className={styles.ccb__bot}>
                <span className={styles.ccb__spanner}>
                  {data.name}
                  <br />
                  {data.name2}
                </span>
                <div className={styles.linkss}>
                  <span className={styles.ccb__span}>{data.person}</span>
                  <Link href={data.link.linker}>{data.link.name}</Link>
                  <Link href={data.phone.linker}>{data.phone.name}</Link>
                </div>
              </div>
            </div>
          ))}
