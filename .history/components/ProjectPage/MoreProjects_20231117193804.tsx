          ...
          <Swiper slidesPerView={3.5} className="mp-swiper" speed={1000}>
            {MoreProjectsData.map((data, i) => (
              <SwiperSlide key={`mp-${i}`}>
                <Link className="mp__slide" href={data.link} style={{height: inView ? "500px" :  0, overflow: "hidden", transition: "height 0.7s cubic-bezier(0.76, 0, 0.24, 1)", transitionDelay: `${0.2*i}s`}}>
                  <Image
                    src={data.image}
                    alt={data.name}
                    fill
