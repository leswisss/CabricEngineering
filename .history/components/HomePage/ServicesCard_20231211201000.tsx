...
const ServicesCard = ({ data, i }: { data: serviceCardProps, i:number }) => {
  ...
  return (
    <div className="services__card-wrapper" ref={ref}  style={{ opacity: activeBack ? 1 : 0, color: activeBack ? "" : "" }}>
      <div className="services__card-cover" style={{height: inView ? "100%" : "0%", transitionDelay: windowWidth > 1000 ? `${i*0.4}s` : "0s"}}>
        ...
        >
          <div className="service__img" style={{ opacity: activeBack ? 1 : 0, color: activeBack ? "" : "" }}>
            <Image
              src={data.image}
              alt={data.title}
              fill
