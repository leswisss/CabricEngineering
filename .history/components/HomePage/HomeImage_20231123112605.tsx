...
const HomeImage = () => {
  ...
  useEffect(() => {
    ...
    spotlightRefs.current.forEach((spotlightRef, i) => {
      ...
      if (inView && spotlightRef && pathRef && textRef) {
        ...
        gsap.set(pathRef, {strokeDasharray: length, strokeDashoffset: length, stroke: 'white'});

        tl.to(spotlightRef, {duration: 0.6, opacity: 1, ease: "power1.inOut"})
          .to(pathRef, {duration: 1, strokeDashoffset: 0, stroke: 'white', ease: "cubic-bezier(0.87, 0, 0.13, 1)"})
  ...
  return (
    <div className="container heroimage__container">
      <div className="hero__image-slider"></div>
      <div className="hero__image" ref={ref}>
        <Image
          src={IMAGE}
          fill
          quality={100}
      ...
      {svgData.map((data, i) => (
        <div className={`room__anim ra-${i + 1}`} key={`svg-${i}`}>
          <div className="spotlight" ref={ref => spotlightRefs.current[i] = ref}>
            <span></span>
          </div>
          <div className="spot__text">
            <svg
              ...
              fill="none"
            >
              <path d={data.path} stroke="transparent" ref={ref => pathRefs.current[i] = ref}/>
            </svg>
            <span ref={ref => textRefs.current[i] = ref}>{data.name}</span>
          </div>
        </div>
      ))}
    </div>
