...
const ConstructionCalculator = () => {
  ...
  const validate = () => {
    ...
    if (!values.area) {
      ...
      newErrors.area = t("areaerror2");
    }

    if (!values.floors) {
      newErrors.floors = t("floorerror");
    } else if (Number(values.floors) < 1) {
      newErrors.floors = t("floorerror2");
    }

    if (!values.bfloors) {
      newErrors.bfloors = t("bfloorerror");
    } else if (values.bfloors !== "" && Number(values.bfloors) < 0) {
      newErrors.bfloors = t("bfloorerror2");
    }

    if (!values.ptype) newErrors.ptype = t("ptypeerror");
    if (!values.quality) newErrors.quality = t("qualityerror");
    if (!values.loca) newErrors.loca = t("locaerror");
  ...
  const calculate = (): CalculationResult => {
    ...
    const ptype = PROJECT_TYPE_LABELS[values.ptype];
    const quality = QUALITY_LABELS[values.quality];
    const location = LOCATION_LABELS[values.loca];
              ...
              <div className={styles.cc__result}>
                ...
                {result.isIndustrial && (
                  <p className={styles.cc__note}>
                    ⚠ This estimate assumes steel frame construction, standard
                    for industrial projects in Cameroon.
                  </p>
                )}
                ...
                <p className={styles.result__disclaimer}>
                  ...
                  fluctuations. For industrial projects, costs assume steel
                  frame construction. Contact CaBric Engineering for a detailed
                  project assessment.
                </p>
              </div>
            )}
      ...
      {activeModal && result && (
        <div className={styles.modal__wrap}>
          <div className={styles.modal}>
            <div className={styles.mo__top}>
              <p className={styles.m__p}>{t("esti")}</p>
              <div className={styles.m__svg}>
                <span
            ...
            <div className={styles.mo__mid}>
              <div className={styles.result__range}>
                <span className={styles.result__label}>{t("estii")}</span>
                <span className={styles.result__value}>
                  {formatXAF(result.low)} - {formatXAF(result.high)}
                  {/* 55.000.000 FCFA - 150.000.000 FCFA */}
                </span>
              </div>
              <div className={styles.result__range}>
                <span className={styles.result__label}>{t("costt")}</span>
                <span className={styles.result__value}>
                  {formatXAF(result.costPerM2)}
                </span>
                {/* <span className={styles.result__value}>650.000 FCFA</span> */}
              </div>
            </div>
            <div className={styles.mo__bottom}>
              <div className={styles.mb__top}>
                <div className={styles.mbt__wrap}>
                  <span className={styles.mb__span}>{t("area2")}</span>
                  <span className={styles.mb__span2}>{result.summary.area}</span>
                </div>
                <div className={styles.mbt__wrap}>
                  <span className={styles.mb__span}>{t("ptype2")}</span>
                  <span className={styles.mb__span2}>{result.summary.ptype}</span>
                </div>
                <div className={styles.mbt__wrap}>
                  <span className={styles.mb__span}>{t("quality2")}</span>
                  <span className={styles.mb__span2}>{result.summary.quality}</span>
                </div>
                <div className={styles.mbt__wrap}>
                  <span className={styles.mb__span}>{t("floors2")}</span>
                  <span className={styles.mb__span2}>{result.summary.floors}</span>
                </div>
                <div className={styles.mbt__wrap}>
                  <span className={styles.mb__span}>{t("bfloors2")}</span>
                  <span className={styles.mb__span2}>{result.summary.bfloors}</span>
                </div>
                <div className={styles.mbt__wrap}>
                  <span className={styles.mb__span}>{t("loca2")}</span>
                  <span className={styles.mb__span2}>{result.summary.location}</span>
                </div>
              </div>
              <div className={styles.disclaim}>
                ...
                {/* Disclaimer */}
                <div className={styles.cc__note2}>
                  <span>
                    <Info />
                  </span>
                  <p>
                    <span className={styles.linkspan}>{t("disclaim")}</span>{" "}
                    <Link
                      className={styles.linkerr}
                      href={currentLng === "en" ? "/contact" : "/fr/contact"}
                    >
                      {t("condis")}
                    </Link>
                  </p>
