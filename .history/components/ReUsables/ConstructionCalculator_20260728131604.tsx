...
const ConstructionCalculator = () => {
  ...
  const validate = () => {
    ...
    if (values.bfloors !== "" && Number(values.bfloors) < 0) {
      newErrors.bfloors = "Basement floors cannot be negative.";
    }
  ...
  const calculate = (): CalculationResult => {
    ...
    const summary = `${QUALITY_LABELS[values.quality]} ${PROJECT_TYPE_LABELS[values.ptype]} · ${area}m² · ${floors} floor${floors > 1 ? "s" : ""}${bfloors > 0 ? ` · ${bfloors} basement` : ""} · ${LOCATION_LABELS[values.loca]}`;
  ...
  const handleReset = () => {
    setValues({ area: "", floors: "", bfloors: "", ptype: "", quality: "", loca: "" });
    setErrors({ area: "", floors: "", bfloors: "", ptype: "", quality: "", loca: "" });
    setResult(null);
  };
  ...
  const formFields = [
    { id: 1, name: "area",   label: t("area"),   type: "number", placeholder: "300", order: 0 },
    { id: 2, name: "floors", label: t("floors"), type: "number", placeholder: "3",   order: 3 },
    { id: 3, name: "bfloors",label: t("bfloors"),type: "number", placeholder: "0",   order: 4 },
  ];

  const otherFields = [
    {
      id: 1, name: "ptype", order: 1, label: t("ptype"),
      error: "Please select a project type.",
      options: [
        { id: "res",    name: t("res")    },
    ...
    {
      id: 2, name: "quality", order: 2, label: t("quality"),
      error: "Please select a quality grade.",
      options: [
        { id: "eco",  name: t("eco")  },
        { id: "stan", name: t("stan") },
    ...
    {
      id: 3, name: "loca", order: 5, label: t("loca"),
      error: "Please select a location.",
      options: [
        { id: "urb",   name: t("urb")   },
        { id: "surb",  name: t("surb")  },
            ...
            <form className={styles.former} onSubmit={handleSubmit}>
              ...
              {values.ptype === "ind" && (
                <p className={styles.cc__note}>
                  ⚠ Industrial estimates assume steel frame construction, which is
                  standard practice in Cameroon and reflects a lower cost per m²
                  than RC frame builds.
                </p>
              )}
          ...
          {result && (
            <div className={styles.cc__result}>
              ...
              <p className={styles.result__disclaimer}>
                ...
                benchmarks for Cameroon and is intended as a planning guide only.
                Actual costs may vary based on site conditions, architectural
                complexity, material choices, and market fluctuations. For
                industrial projects, costs assume steel frame construction.
                Contact CaBric Engineering for a detailed project assessment.
              </p>
            </div>
          )}
        </div>
