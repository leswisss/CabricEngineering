          ...
          {leadContent.map((data, i) => (
            <div className={styles.lead__wrap} key={i}>
              <div className={styles.image__wrap}>
                <Image fill quality={100} alt={`${data.name} - CaBric Engineering`} src={data.image} placeholder="blur"/>
              </div>
              <div className={styles.lead__mid}>
                <p>{data.name}</p>
                <span>{t("about:co")}</span>
              </div>
              <div className={styles.lead__bot}>
                <p>{data.text}</p>
                <Link className={styles.linkedin} target="_blank" href={data.linkedIn}>LinkedIn</Link>
              </div>
            </div>
          ))}
        </div>
