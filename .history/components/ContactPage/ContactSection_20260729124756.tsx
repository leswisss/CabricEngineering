...
const ContactSection = () => {
  ...
  // FIX 1: Store country as a plain string (the name), not the full object
  const [countryName, setCountryName] = useState<string>("");

  // FIX 2: Store phone value in state so submission uses the validated value
  ...
  const handlePhoneChange = (value: string, country: CountryData) => {
    // FIX 2: Store the phone value in state
    setPhoneValue(value);
  ...
  const toggleCheckbox = () => {
    const newActiveBox = !activeBox;
    setActiveBox(newActiveBox);
    if (newActiveBox) setTickError(false);
  };
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // FIX 2: Use phoneValue from state, not formData (more reliable with react-phone-input-2)
    const phone = phoneValue;

    // FIX 1: Use countryName (already a plain string)
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error("Server error:", errorBody?.message ?? response.status);
        setButtonText(t("sorry"));
      }
    } catch (error) {
        ...
        <div className={styles.con__right}>
          ...
          >
            ...
            <div className={styles.form__top}>
              ...
              <div className={styles.form__in}>
                <span className={styles.label}>{t("country")}</span>
                {/* FIX 1: Extract only the country name string */}
                <CountrySelect
                  containerClassName="form-group"
                  inputClassName=""
                  onChange={(_country: any) =>
                    setCountryName(_country?.name ?? "")
                  }
                  onTextChange={(_txt) => console.log(_txt)}
                  placeHolder={t("yourcountry")}
                />
