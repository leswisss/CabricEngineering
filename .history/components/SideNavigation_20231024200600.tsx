...
import { sidebarcontent } from '@/utils'import { sideBarProps } from '@/types';
      ...
      <div className="s-links__container">
        {
          sidebarcontent.map((link, i) => (
            <Link href={link.href} key={`sl${i}`} onMouseOver={() => setActiveLink(i)} onMouseOut={() => setActiveLink(0)}>{link.name}</Link>
          ))
        }
      </div>
    </div>
