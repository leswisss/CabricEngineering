...
const Preloader = ({isInitial} : {isInitial?:boolean}) => {
  ...
  useLayoutEffect(() => {
    ...
    gsap.to(refs.current, {
      ...
      duration: 0.7,
      delay: 1,
      onComplete: () => setOnComplete1(true),
    });
  }, []);
  ...
  return (
    <section
      className="preloader"
      style={{ display: onComplete2 ? "none" : "block" }}
    >
      <div className="preloader__text">
        ...
        <div className="preloader-content">
          {preloaderData.map((data, i) => {
            const shouldDisplay = data.path === "/portfolio/" ? pathname.includes(data.path) : data.path === "/" ? pathname === data.path && !isInitial  :pathname === data.path;
            return (
              <h2
                key={`pc-${i}`}
